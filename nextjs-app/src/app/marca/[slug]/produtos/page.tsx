...
export default function ProdutoDetalhePage() {
  ...
  const summaryByCategory = useMemo(() => {
    const map = new Map<
      string,
      { count: number; totalPieces: number; totalPrice: number; prices: number[] }
    >();
    activeProducts.forEach((p) => {
      const q = getQty(p.id);
      const pieces = p.sizes.length * p.variants.length * q;
      const existing =
        map.get(p.category) || { count: 0, totalPieces: 0, totalPrice: 0, prices: [] };
      existing.count++;
  ...
  const summaryByGender = useMemo(() => {
    ...
    activeProducts.forEach((p) => {
      ...
      const pieces = p.sizes.length * p.variants.length * q;
      const existing =
        map.get(p.gender) || { count: 0, totalPieces: 0, totalPrice: 0, prices: [] };
      existing.count++;
      existing.totalPieces += pieces;
      existing.totalPrice += pieces * p.price;
      existing.prices.push(p.price);
  ...
  const grandTotal = activeProducts.reduce(
    (a, p) => a + p.sizes.length * p.variants.length * getQty(p.id),
    0,
  );
  const grandPrice = activeProducts.reduce(
    (a, p) => a + p.sizes.length * p.variants.length * getQty(p.id) * p.price,
    0,
  );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        <NextilSidebar />
        <div
          className={`flex-1 min-w-0 pb-16 md:pb-0 transition-all duration-300 ${
            sideMode && selectedProduct ? "mr-[380px]" : ""
          }`}
        >
          ...
          <div className="sticky top-14 md:top-16 z-30 bg-card border-b border-border">
            <div className="px-3 md:px-6 py-2 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                <button
                  onClick={() => router.push(`/marca/${brand.slug}`)}
            ...
            <div className="px-3 md:px-6 py-2 flex items-center justify-between gap-2 bg-background/80">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <Button
                  ...
                  className="gap-1 text-xs h-7 px-2.5 rounded-md"
                >
                ...
                {filteredProducts.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="p-2 md:p-3">
                      ...
                      <p className="text-[10px] md:text-xs text-foreground mt-0.5 md:mt-1 line-clamp-2 leading-snug">
                        {p.name}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-0.5 mt-1">
                        {p.sizes.map((s) => (
                          <span
                            key={s}
                            className="px-1 py-0.5 rounded bg-muted text-[7px] md:text-[8px] font-medium text-muted-foreground"
                          >
                            {s}
                          </span>
                        ))}
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {activeProducts.length} de {filteredProducts.length} produtos · Grade padrão (1
                    peça por tamanho/cor)
                  </p>
                </div>
                <button
                  onClick={() => setAddAllModal(false)}
              ...
              <div className="flex-1 overflow-y-auto">
                        ...
                        <tbody>
                          {Array.from(summaryByCategory.entries()).map(([cat, data]) => (
                            <tr key={cat} className="border-t border-border">
                              ...
                              <td className="px-2 py-2 text-center text-muted-foreground">
                                {grandTotal > 0
                                  ? ((data.totalPieces / grandTotal) * 100).toFixed(1)
                                  : 0}
                                %
                              </td>
                            ...
                            .map(([size, pieces]) => (
                              <tr key={size} className="border-t border-border">
                                ...
                                <td className="px-3 py-2 text-right text-muted-foreground">
                                  {grandTotal > 0
                                    ? ((pieces / grandTotal) * 100).toFixed(1)
                                    : 0}
                                  %
                                </td>
                              </tr>
                ...
                <div className="px-3 md:px-5 py-3">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mb-2 md:mb-3">
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center gap-2 md:gap-3 py-1.5 md:py-2 px-1.5 md:px-2 rounded-lg border transition-all ${
                            isExcluded ? "border-border/50 bg-muted/30 opacity-50" : "border-border"
                          }`}
                        >
                          <img
                            src={p.variants[0]?.images[0]}
                          ...
                          {!isExcluded && (
                            <div className="flex items-center gap-0.5 md:gap-1 shrink-0">
                              ...
                              <span className="w-5 md:w-6 text-center text-[10px] md:text-xs font-bold text-foreground">
                                {q}
                              </span>
                              <button
                                onClick={() =>
              ...
              <div className="shrink-0 border-t border-border bg-muted/30 px-3 md:px-5 py-2.5 md:py-3 flex items-center justify-between gap-3">
                ...
                <Button
                  ...
                  className="gap-1.5 shrink-0 text-xs md:text-sm"
