...
const sidebarItems: { icon: typeof Home; label: string; path: `/${string}` }[] = [
      ...
      <header className="sticky top-0 z-50 flex h-14 md:h-16 items-center justify-between border-b border-sidebar-border bg-header px-3 md:px-6 gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={() => router.push("/")} className="flex items-center gap-2">
            <img src={typeof nextilLogo === "string" ? nextilLogo : nextilLogo.src} alt="Nextil" className="h-6 w-6 md:h-7 md:w-7" />
            <img src={typeof nextilWordmark === "string" ? nextilWordmark : nextilWordmark.src} alt="Nextil" className="h-4 md:h-5 hidden sm:block" />
          </button>
        </div>
