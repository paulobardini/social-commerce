...
import {
  ...
  Pin, Trash2, ThumbsUp, ExternalLink, Star,
} from "lucide-react";
import {
  mockClientes360, mockConversas, mockMensagens, mockPedidos, mockNotas,
  mockHistorico, mockTarefas360, statusLabels, statusColors, nichoLabels,
  temperaturaColors, tipoTarefaLabels, type Cliente360,
...
export default function Cliente360Page() {
  ...
  const oportunidades = mockOportunidades.filter(o => {
    const clienteMap: Record<string, string> = { c1: "c7", c2: "c11", c3: "c4", c4: "c12", c5: "c8", c6: "c9", c7: "c3", c8: "c13", c9: "c14", c10: "c16", c11: "c15", c12: "c17" };
    return o.clienteNome === cliente.nomeFantasia;
  });
        ...
        <Tabs value={tab} onValueChange={setTab} className="space-y-4">
          ...
          <TabsContent value="resumo" className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Oportunidades abertas", value: cliente.oportunidadesAbertas, color: "text-purple-600" },
                { label: "Orçamentos ativos", value: cliente.orcamentosAtivos, color: "text-blue-600" },
                { label: "Pedidos realizados", value: cliente.pedidosRealizados, color: "text-green-600" },
                { label: "Tarefas pendentes", value: tarefas.filter(t => t.status !== "concluida").length, color: "text-orange-600" },
              ].map(k => (
                <Card key={k.label} className="border border-border">
                  <CardContent className="p-4">
                    <p className={`text-2xl font-bold font-heading ${k.color}`}>{k.value}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <Card className="border border-border">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-heading">Informações</CardTitle></CardHeader>
                <CardContent className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-muted-foreground">Origem</span><span className="font-medium">{cliente.origem}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Data cadastro</span><span className="font-medium">{cliente.dataCadastro}</span></div>
                  ...
                  <div className="flex justify-between"><span className="text-muted-foreground">Marcas de interesse</span><span className="font-medium">{cliente.marcasInteresse.join(", ")}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Próxima ação</span><span className="font-medium text-accent">{cliente.proximaAcao}</span></div>
                </CardContent>
              </Card>
              <Card className="border border-border">
                <CardHeader className="pb-2"><CardTitle className="text-sm font-heading">Últimas movimentações</CardTitle></CardHeader>
            ...
            <div className="border border-border rounded-lg overflow-hidden h-[500px] flex">
              ...
              <div className="flex-1 flex flex-col">
                <div className="h-14 border-b border-border flex items-center px-4 gap-3 bg-card">
                  <div className="h-9 w-9 rounded-full bg-green-100 flex items-center justify-center"><MessageCircle className="h-4 w-4 text-green-600" /></div>
                  <div>
                ...
                {orcamentos.map(orc => (
                  <div key={orc.id} className="flex items-center justify-between p-4 border border-border rounded-lg hover:bg-muted/30 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button variant="outline" size="sm" onClick={() => navigate(`/vendedor/orcamento/${orc.id}`)}>Abrir</Button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <TabsContent value="pedidos" className="space-y-4">
            <p className="text-sm text-muted-foreground">{pedidos.length} pedidos registrados</p>
            {pedidos.length > 0 ? (
              <div className="space-y-2">
                {pedidos.map(p => (
          ...
          <TabsContent value="tarefas" className="space-y-4">
            ...
            <div className="space-y-2">
              {tarefas.map(t => (
                <div key={t.id} className={`flex items-center justify-between p-3 rounded-lg border transition-colors ${t.status === "atrasada" ? "border-red-200 bg-red-50/50" : t.status === "concluida" ? "border-green-200 bg-green-50/50" : "border-border"}`}>
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{tipoTarefaLabels[t.tipo]} · {t.vencimento}</p>
                    </div>
                  </div>
                  <Badge variant={t.prioridade === "alta" ? "destructive" : "secondary"} className="text-[10px] shrink-0">{t.prioridade}</Badge>
                </div>
              ))}
              {tarefas.length === 0 && (
              ...
              return (
                <div key={h.id} className="flex gap-3 items-start">
                  <div className="flex flex-col items-center">
                    <div className={`h-7 w-7 rounded-full flex items-center justify-center ${historicoColors[h.tipo]}`}>
                  ...
                  <div className="pb-4">
                    ...
                    {h.detalhes && <p className="text-xs text-muted-foreground mt-0.5 italic">{h.detalhes}</p>}
          ...
          <TabsContent value="anotacoes" className="space-y-4">
            <div className="flex gap-2">
              <Textarea placeholder="Adicionar uma nota..." value={notaInput} onChange={e => setNotaInput(e.target.value)} className="min-h-[60px]" />
            ...
            <div className="space-y-2">
              {notas.sort((a, b) => (b.fixada ? 1 : 0) - (a.fixada ? 1 : 0)).map(n => (
                <div key={n.id} className={`p-3 rounded-lg border transition-colors ${n.fixada ? "border-yellow-200 bg-yellow-50/50" : "border-border"}`}>
